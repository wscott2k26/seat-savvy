    ...
    <div className="safe-screen relative h-full w-full overflow-y-auto bg-[radial-gradient(circle_at_18%_0%,rgba(214,168,79,0.13),transparent_26%),linear-gradient(180deg,#030712_0%,#0a1022_40%,#1b1126_100%)] text-[#f8edd2]">
      <header className="safe-header relative overflow-hidden rounded-b-[34px] border-b border-[#d6a84f]/22 bg-[linear-gradient(145deg,rgba(4,9,20,0.98),rgba(24,20,42,0.96)_52%,rgba(8,23,38,0.98))] pb-5 shadow-[0_24px_58px_rgba(0,0,0,0.54),0_0_36px_rgba(214,168,79,0.11)]">
        ...
        <div className="relative flex items-center justify-between">
          <button onClick={openMenu} className="safe-hit grid place-items-center rounded-full border border-white/10 bg-white/10 text-[#fff5d8] shadow-lg ring-1 ring-[#d6a84f]/15 active:scale-95" type="button">
          ...
          <button onClick={openShop} className="rounded-full bg-gradient-to-r from-[#d6a84f] to-[#f0c76a] px-4 py-2 text-sm font-black text-[#15101f] shadow-[0_12px_28px_rgba(214,168,79,0.24)] active:scale-95" type="button">
            Visit Shop
          </button>
        </div>
        <p className="relative mt-4 text-xs font-black uppercase tracking-[0.18em] text-[#d6a84f]">Home Studio</p>
        <h1 className="relative font-display text-3xl font-black text-[#fff5d8]">{home.label}</h1>
        <p className="relative mt-1 text-sm font-semibold leading-relaxed text-[#d9cda9]">{home.description}</p>
        <div className="relative mt-4 grid grid-cols-3 gap-2 text-center text-[11px] font-black text-[#f6d98d]">
          ...
          <span className="rounded-2xl border border-[#d6a84f]/22 bg-[#d6a84f]/10 px-2 py-2">{homeStats.yard}</span>
      ...
      <main className="safe-content space-y-5 pt-5">
        <section className="rounded-[30px] border border-white/10 bg-white/8 p-2 shadow-[0_22px_44px_rgba(0,0,0,0.32)]">
          <div className="grid grid-cols-2 gap-2">
            {(['inside', 'outside'] as SceneMode[]).map((mode) => (
              <button key={mode} onClick={() => setSceneMode(mode)} className={`rounded-2xl px-3 py-2 text-sm font-black capitalize active:scale-95 ${sceneMode === mode ? 'bg-[#d6a84f] text-[#15101f]' : 'border border-white/10 bg-[#071022]/70 text-[#d9cda9]'}`} type="button">
                {mode}
              </button>
            ))}
          </div>
        ...
        <section className="rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(9,16,33,0.88),rgba(6,12,26,0.76))] p-4 shadow-[0_22px_44px_rgba(0,0,0,0.38),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {HOME_TRAY_CATEGORIES.map((tray) => (
              <button key={tray.id} onClick={() => setCategory(tray.id)} className={`shrink-0 rounded-full px-3 py-2 text-xs font-black shadow transition active:scale-95 ${category === tray.id ? 'bg-[#d6a84f] text-[#15101f]' : 'border border-white/10 bg-white/8 text-[#d9cda9]'}`} type="button">
                {tray.label}
              </button>
            ))}
          </div>
          <div className="mt-3 grid grid-cols-2 gap-3">
            {visibleItems.map((item) => (
              <DecorCard key={item.id} equipped={equipped.has(item.id) || progress.life.selectedPet === item.id} item={item} owned={owned.has(item.id)} stars={stars} onClick={() => handleItem(item)} />
            ))}
          </div>
        </section>
        ...
        <section className="rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(9,16,33,0.88),rgba(6,12,26,0.76))] p-4 shadow-[0_22px_44px_rgba(0,0,0,0.38),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur">
          ...
          <p className="text-xs font-semibold text-[#a9a0b5]">Every move gets bigger: more rooms, richer interiors, and better outside space.</p>
          <div className="mt-3 space-y-3">
            {HOME_UPGRADES.map((candidate) => (
              <HomeUpgradeCard key={candidate.id} active={candidate.id === progress.life.homeId} canAfford={progress.coins >= candidate.cost} home={candidate} lockedPremium={!!candidate.premium && !progress.premium} lockedStars={!!candidate.starsRequired && stars < candidate.starsRequired} owned={ownedHomes.has(candidate.id)} onClick={() => upgradeHome(candidate.id)} />
            ))}
          </div>
        </section>
      </main>
...
const ExteriorScene: React.FC<{ equipped: Set<string>; home: HomeUpgrade; mood: HomeMood }> = ({ equipped, home, mood }) => {
  ...
  return (
    <>
      ...
      {equipped.has('fence-style') && yardAllows(yard, 'yard') && <FenceLine />}
      {equipped.has('garden-path') && yardAllows(yard, 'patio') && <GardenPath />}
      <HomeFacade homeId={home.id} />
      {equipped.has('flower-planter') && <OutdoorPlanters />}
      {equipped.has('mailbox') && yard !== 'rooftop' && <Mailbox />}
      {equipped.has('porch-chair') && <PorchChair />}
      {equipped.has('porch-swing') && yardAllows(yard, 'deck') && <PorchSwing />}
      {equipped.has('fire-pit') && yardAllows(yard, 'deck') && <OutdoorFirePit />}
      {equipped.has('grill') && yardAllows(yard, 'yard') && <Grill />}
      {equipped.has('bbq-smoker') && yardAllows(yard, 'yard') && <BbqSmoker />}
      {equipped.has('outdoor-kitchen') && yardAllows(yard, 'rooftop') && <OutdoorKitchen />}
      {equipped.has('patio-dining-set') && yardAllows(yard, 'yard') && <PatioDiningSet />}
      {equipped.has('umbrella-table') && yardAllows(yard, 'deck') && <UmbrellaTable />}
      {equipped.has('lounge-chairs') && yardAllows(yard, 'deck') && <LoungeChairs />}
      {equipped.has('garden-box') && yardAllows(yard, 'yard') && <GardenBox />}
      {equipped.has('gazebo') && yardAllows(yard, 'estate') && <Gazebo />}
      {equipped.has('outdoor-tv') && yardAllows(yard, 'rooftop') && <OutdoorTv />}
      {equipped.has('backyard-pool') && yardAllows(yard, 'estate') && <BackyardPool />}
      {equipped.has('pool-deck') && yardAllows(yard, 'estate') && <PoolDeck />}
      {equipped.has('hot-tub') && yardAllows(yard, 'rooftop') && <HotTub />}
      {equipped.has('pool-lights') && yardAllows(yard, 'estate') && <PoolLights />}
      {equipped.has('outside-string-lights') && yardAllows(yard, 'patio') && <OutdoorStringLights />}
      <YardBadge yard={yard} />
    </>
  );
};
...
function homeMood(homeId: string, equipped: Set<string>): HomeMood {
  ...
  const moods: Record<string, HomeMood> = {
    ...
    'cozy-cabin': { insideKicker: 'Warm timber', outsideKicker: 'Forest deck', view: 'mountain', wall: 'linear-gradient(135deg,#23170f,#4b2f1b 54%,#151018)', floor: 'repeating-linear-gradient(105deg,#7a4d27 0 22px,#5d381e 22px 44px)', sky: 'linear-gradient(180deg,#263848,#52665b 60%,#17251f)', ground: 'linear-gradient(180deg,#31472e,#172512)' },
    'beach-cottage': { insideKicker: 'Ocean light', outsideKicker: 'Beach porch', view: 'ocean', wall: 'linear-gradient(135deg,#0e2330,#22445a 54%,#101827)', floor: 'repeating-linear-gradient(105deg,#8c6b43 0 22px,#6f5337 22px 44px)', sky: 'linear-gradient(180deg,#98c7d9,#398aa4 58%,#165069)', ground: 'linear-gradient(180deg,#ccb27a,#8d7042)' },
    'lake-house': { insideKicker: 'Quiet water', outsideKicker: 'Deck and dock', view: 'lake', wall: 'linear-gradient(135deg,#12211f,#26352d 54%,#0d1724)', floor: 'repeating-linear-gradient(105deg,#73512f 0 22px,#52371f 22px 44px)', sky: 'linear-gradient(180deg,#516979,#3c6870 58%,#102d34)', ground: 'linear-gradient(180deg,#2d5d5d,#173334)' },
    'city-loft': { insideKicker: 'Skyline loft', outsideKicker: 'Rooftop patio', view: 'skyline', wall: 'linear-gradient(135deg,#070d19,#20273b 46%,#120d1f)', floor: 'repeating-linear-gradient(105deg,#6a4b30 0 26px,#493321 26px 52px)', sky: 'linear-gradient(180deg,#071025,#1f3760 58%,#040713)', ground: 'linear-gradient(180deg,#111827,#030712)' },
    'garden-villa': { insideKicker: 'Garden rooms', outsideKicker: 'Villa yard', view: 'garden', wall: 'linear-gradient(135deg,#17251f,#40543e 52%,#102018)', floor: 'repeating-linear-gradient(105deg,#80623d 0 22px,#5f472a 22px 44px)', sky: 'linear-gradient(180deg,#5b7a8d,#9bb38f 58%,#315044)', ground: 'linear-gradient(180deg,#5f8c4c,#25401f)' },
    'luxury-penthouse': { insideKicker: 'Full Adventure', outsideKicker: 'Rooftop luxury', view: 'skyline', wall: 'linear-gradient(135deg,#050816,#171122 46%,#1d1731)', floor: 'repeating-linear-gradient(105deg,#87633b 0 24px,#604627 24px 48px)', sky: 'linear-gradient(180deg,#050816,#1c2f57 60%,#030712)', ground: 'linear-gradient(180deg,#101827,#050816)' },
    'dream-estate': { insideKicker: 'Estate grand room', outsideKicker: 'Pool yard estate', view: 'estate', wall: 'linear-gradient(135deg,#110d1b,#2d243c 50%,#071022)', floor: 'repeating-linear-gradient(105deg,#a17a43 0 24px,#76552f 24px 48px)', sky: 'linear-gradient(180deg,#0b1530,#304f7a 58%,#050816)', ground: 'linear-gradient(180deg,#4f7142,#17321f)' },
  };
...
function yardTierForHome(homeId: string): YardTier {
  ...
  if (homeId === 'lake-house' || homeId === 'beach-cottage' || homeId === 'cozy-cabin') return 'deck';
  if (homeId === 'suburban-house' || homeId === 'garden-villa') return 'yard';
  if (homeId === 'small-trailer' || homeId === 'starter-apartment' || homeId === 'city-apartment') return 'patio';
  return 'stoop';
}
...
const Piano = () => <div className="absolute left-[8%] bottom-[37%] h-14 w-28 rounded-t-2xl bg-[#100d12] shadow-xl ring-1 ring-[#d6a84f]/18" />;
...
const PorchChair = () => <div className="absolute bottom-[25%] right-[9%] h-16 w-16"><span className="absolute bottom-2 h-8 w-14 rounded-xl bg-[#31536c]" /><span className="absolute left-3 top-0 h-11 w-10 rounded-t-2xl bg-[#4f6f8a]" /></div>;
const PorchSwing = () => <div className="absolute bottom-[31%] left-[13%] h-16 w-28"><span className="absolute left-2 top-0 h-16 w-px bg-[#d6a84f]/60" /><span className="absolute right-2 top-0 h-16 w-px bg-[#d6a84f]/60" /><span className="absolute bottom-0 h-8 w-full rounded-2xl bg-[#5d4026] shadow-xl" /></div>;
...
const Grill = () => <div className="absolute bottom-[22%] right-[29%] h-16 w-20"><span className="absolute bottom-4 h-10 w-full rounded-b-2xl rounded-t-full bg-[#1f2638] shadow-xl" /><span className="absolute bottom-0 left-4 h-5 w-1.5 bg-[#a9a0b5]" /><span className="absolute bottom-0 right-4 h-5 w-1.5 bg-[#a9a0b5]" /></div>;
...
const GardenBox = () => <div className="absolute bottom-[17%] left-[8%] h-12 w-28 rounded-xl bg-[#5d4026] shadow-xl"><span className="absolute bottom-8 left-3 h-7 w-5 rounded-full bg-[#729d58]" /><span className="absolute bottom-8 left-10 h-8 w-5 rounded-full bg-[#87aa67]" /><span className="absolute bottom-8 right-4 h-7 w-5 rounded-full bg-[#729d58]" /></div>;
const Gazebo = () => <div className="absolute bottom-[24%] left-[8%] h-28 w-32"><span className="absolute top-0 h-8 w-full bg-[#eadfcb]/70" style={{ clipPath: 'polygon(50% 0,100% 100%,0 100%)' }} /><span className="absolute bottom-0 left-5 h-20 w-2 bg-[#eadfcb]/60" /><span className="absolute bottom-0 right-5 h-20 w-2 bg-[#eadfcb]/60" /><span className="absolute bottom-0 left-3 right-3 h-5 rounded bg-[#5d4026]" /></div>;
const OutdoorTv = () => <div className="absolute bottom-[36%] right-[8%] h-16 w-28 rounded-xl bg-[#050816] shadow-xl ring-4 ring-[#394b63]"><span className="absolute inset-3 rounded bg-[#1f3760]" /></div>;
const BackyardPool = () => <div className="absolute bottom-[5%] left-[16%] right-[16%] h-24 rounded-[50%] bg-[#2fa0c6]/55 shadow-[0_0_24px_rgba(47,160,198,0.32)] ring-4 ring-[#eadfcb]/35" />;
const PoolDeck = () => <div className="absolute bottom-[3%] left-[10%] right-[10%] h-32 rounded-[44px] border-8 border-[#a17a43]/55" />;
...
const OutdoorStringLights = () => <div className="absolute left-[10%] right-[10%] top-[12%] flex justify-between">{Array.from({ length: 10 }).map((_, i) => <span key={i} className="h-3 w-3 rounded-full bg-[#f6d98d] shadow-[0_0_18px_rgba(246,217,141,0.75)]" />)}</div>;
const YardBadge: React.FC<{ yard: YardTier }> = ({ yard }) => <div className="absolute bottom-4 left-4 rounded-2xl border border-white/10 bg-[#030712]/48 px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-[#d6a84f] shadow-xl backdrop-blur">{yard} space</div>;

const DecorCard: React.FC<{ equipped: boolean; item: ShopItem; owned: boolean; stars: number; onClick: () => void }> = ({ equipped, item, owned, stars, onClick }) => { const lockedForStars = !!item.starsRequired && stars < item.starsRequired; const price = shopItemPriceLabel(item); const status = owned ? (equipped ? 'Equipped' : 'Owned') : item.premium ? 'Premium preview' : lockedForStars ? `${item.starsRequired} stars` : `${item.cost ?? 0} coins`; return <button onClick={onClick} className={`min-h-[136px] rounded-[24px] border p-3 text-left shadow-[0_16px_28px_rgba(0,0,0,0.3)] transition active:scale-[0.98] ${equipped ? 'border-[#d6a84f]/60 bg-[#d6a84f]/16' : owned ? 'border-white/12 bg-white/8' : 'border-white/8 bg-[#040816]/58 opacity-85'}`} type="button"><div className="mb-3 h-12 overflow-hidden rounded-2xl border border-white/10 bg-[linear-gradient(135deg,#151d2e,#31213c)] shadow-inner"><ShopItemPreview item={item} /></div><p className="font-display text-sm font-black leading-tight text-[#fff5d8]">{item.label}</p><p className="mt-1 line-clamp-2 text-[11px] font-semibold leading-snug text-[#a9a0b5]">{item.description}</p><div className="mt-2 flex flex-wrap gap-2"><span className="inline-flex rounded-full bg-black/30 px-2 py-1 text-[10px] font-black text-[#f6d98d]">{price}</span><span className="inline-flex rounded-full border border-white/10 bg-white/8 px-2 py-1 text-[10px] font-black text-[#d9cda9]">{status}</span></div></button>; };
const HomeUpgradeCard: React.FC<{ active: boolean; canAfford: boolean; home: HomeUpgrade; lockedPremium: boolean; lockedStars: boolean; owned: boolean; onClick: () => boolean }> = ({ active, canAfford, home, lockedPremium, lockedStars, owned, onClick }) => { const status = active ? 'Current' : owned ? 'Move in' : lockedPremium ? 'Premium' : lockedStars ? 'Locked' : !canAfford ? 'Need coins' : 'Buy'; const statusClass = active ? 'bg-[#d6a84f] text-[#15101f]' : owned ? 'bg-[#b7d6c8] text-[#102018]' : lockedStars || lockedPremium || !canAfford ? 'border border-white/10 bg-white/8 text-[#a9a0b5]' : 'bg-[#f6d98d] text-[#15101f]'; return <button onClick={onClick} className={`w-full overflow-hidden rounded-[24px] border p-3 text-left shadow-[0_16px_30px_rgba(0,0,0,0.32)] transition active:scale-[0.99] ${active ? 'border-[#d6a84f]/70 bg-[#d6a84f]/18' : owned ? 'border-[#b7d6c8]/36 bg-[#b7d6c8]/10' : 'border-white/10 bg-white/8'}`} type="button"><div className="flex gap-3"><div className="relative h-16 w-20 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-[#050816]"><MiniRoom homeId={home.id} /></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><span className="font-display text-sm font-black text-[#fff5d8]">{home.label}</span><span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-black ${statusClass}`}>{status}</span></div><p className="mt-1 line-clamp-2 text-xs font-semibold text-[#a9a0b5]">{home.description}</p><p className="mt-2 text-[10px] font-black uppercase tracking-wide text-[#d6a84f]">{owned ? 'Owned' : homePriceLabel(home)}{lockedStars && !owned ? ' / locked' : ''}</p></div></div></button>; };
const MiniRoom: React.FC<{ homeId: string }> = ({ homeId }) => { const mood = homeMood(homeId, new Set()); return <div className="relative h-full w-full" style={{ background: mood.wall }}><div className="absolute inset-x-0 bottom-0 h-7" style={{ background: mood.floor }} /><span className="absolute left-2 top-2 h-8 w-10 rounded-xl bg-[#9fb6d9]/38 ring-1 ring-white/20" /><span className="absolute bottom-3 left-3 h-4 w-11 rounded-lg bg-[#6a4a75]" /><span className="absolute bottom-4 right-3 h-2 w-8 rounded bg-[#8a5a2b]" />{['luxury-penthouse', 'dream-estate'].includes(homeId) && <span className="absolute right-2 top-2 h-8 w-5 rounded bg-[#d6a84f]/30" />}</div>; };
function homeCategoryFor(item: ShopItem): HomeTrayCategory { if (item.kind === 'pet') return 'pets'; if (item.kind === 'view' || item.kind === 'wallpaper' || item.kind === 'floor') return 'views'; if (item.category === 'kitchen') return 'kitchen'; if (item.category === 'bathroom') return 'bathroom'; if (item.category === 'outdoor') return 'outdoor'; if (item.category === 'lighting') return 'lighting'; if (['cozy-bed', 'luxury-bed', 'pet-bed', 'bath-mat'].includes(item.id)) return 'bedroom'; if (['starter-plant', 'bookshelf', 'wall-art', 'round-mirror', 'trophy-shelf', 'aquarium', 'cozy-rug', 'piano', 'jukebox', 'telescope', 'spa-candles'].includes(item.id)) return 'decor'; return 'living'; }
function viewFor(equipped: Set<string>, homeId: string): string { if (equipped.has('skyline-view')) return 'skyline'; if (equipped.has('lake-view')) return 'lake'; if (equipped.has('ocean-view')) return 'ocean'; if (equipped.has('mountain-view')) return 'mountain'; if (homeId === 'city-apartment' || homeId === 'city-loft' || homeId === 'luxury-penthouse') return 'skyline'; if (homeId === 'beach-cottage') return 'ocean'; if (homeId === 'lake-house') return 'lake'; if (homeId === 'cozy-cabin') return 'mountain'; if (homeId === 'garden-villa') return 'garden'; if (homeId === 'dream-estate') return 'estate'; if (homeId === 'small-trailer' || homeId === 'suburban-house') return 'neighborhood'; return 'rain'; }
